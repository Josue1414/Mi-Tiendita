...
import { X, Tag, Package, Layers, MonitorUp, MonitorOff, Edit, Barcode, ArrowUp, Trash2 } from "lucide-react";
...
interface Props {
  ...
  alCerrar: () => void;
  // Permisos y acciones opcionales para usarlo en Inventario
  puedeEditar?: boolean;
  ...
  alEliminar?: () => void;
  alAjustarStock?: () => void;
  alVerCodigoBarras?: () => void;
}

export default function ModalDescripcionProducto({ 
  abierto, producto, alCerrar, 
  puedeEditar, puedeEliminar, puedeAjustarStock, 
  alEditar, alEliminar, alAjustarStock, alVerCodigoBarras 
}: Props) {
      ...
      <div className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-slate-900" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-[170px,1fr]">
          ...
          <div className="flex flex-col">
            ...
            {/* SECCIÓN DE BOTONES DE ACCIÓN PARA INVENTARIO */}
            {(alEditar || alEliminar || alAjustarStock || alVerCodigoBarras) && (
              <div className="mt-4 flex flex-wrap gap-2 pt-4 border-t border-slate-200 dark:border-white/10">
                {(puedeEditar || puedeAjustarStock) && alEditar && (
                  <button onClick={alEditar} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-slate-100 px-3 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 transition-colors">
                    <Edit size={14} /> Editar
                  </button>
                )}
                {alVerCodigoBarras && (
                  <button onClick={alVerCodigoBarras} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-slate-100 px-3 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 transition-colors">
                    <Barcode size={14} /> Etiqueta
                  </button>
                )}
                {(puedeEditar || puedeAjustarStock) && alAjustarStock && (
                  <button onClick={alAjustarStock} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-50 px-3 py-2.5 text-xs font-bold text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 dark:hover:bg-emerald-900/50 transition-colors">
                    <ArrowUp size={14} /> Stock
                  </button>
                )}
                {puedeEliminar && alEliminar && (
                  <button onClick={alEliminar} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-red-50 px-3 py-2.5 text-xs font-bold text-red-700 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50 transition-colors">
        ...
        <div className="flex justify-end items-center gap-2 border-t border-slate-200 px-5 py-4 dark:border-white/10">
          <button onClick={alternarPantallaCliente} className="inline-flex items-center gap-2 rounded-xl bg-slate-800 px-4 py-2.5 text-sm font-bold text-white hover:bg-slate-700 transition-colors">
            ...
            {mostrandoEnPantallaCliente ? "Dejar de mostrar en cliente" : "Mostrar en cliente"}
          </button>
